...
class DataController extends Controller
{
    ...
    {
        //
        $data = DB::select("Select ed.county, ed.period, ed.data_group, ed.data_sub_group, ed.indicator, ed.score from equity_data ed join ( select distinct t.county, t.period, t.indicator, t.data_group, t.data_sub_group, equity_data.upload_id from ( SELECT distinct county, period, data_group, data_sub_group, indicator, MAX(updated_at) OVER ( PARTITION BY county, period, indicator, data_group, data_sub_group ) FROM public.equity_data ) as t inner join equity_data on t.county = equity_data.county and t.period = equity_data.period and t.data_group = equity_data.data_group and t.data_sub_group = equity_data.data_sub_group and t.indicator = equity_data.indicator and t.max = equity_data.updated_at ) a on ed.county = a.county and ed.period = a.period and ed.data_group = a.data_group and ed.data_sub_group = a.data_sub_group and ed.indicator = a.indicator and ed.upload_id = a.upload_id");
        return response()->json( $data,200);
    }
